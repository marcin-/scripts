...
class QueueManager:
    ...
    def __del__(self):
        self.__serialize(self.waitQueue, self.waitQueueFilePath)
        self.__serialize(self.workQueue, self.workQueueFilePath)

    def __serialize(self, queueName, filePath):
        try:
            queue = open(filePath, "w")
        except IOError:
            return
